  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        const msg = [];
        msg.push(buildValidationMessage(body?.errors));
        msg.push('Please, try again.')
      ...
      if (response.status === 409) {
        const msg =
          //body?.details ||
          "Error: A resource with the same name already exists. Please choose another name.";
    ...
    const msgLines = [];
    if (createdAt) msgLines.push(`Great! Resources added successfully:`);
    msgLines.push(`Name = "${body?.data?.name ?? ""}"`);
    msgLines.push(`Price = "${body?.data?.price ?? ""}"`);
    msgLines.push(`Unit = "${body?.data?.price_unit ?? ""}"`);
