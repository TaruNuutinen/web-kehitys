...
function isResourceDescriptionValid(value) { const trimmed = value.trim(); const allowedPattern = /^[a-zA-Z0-9äöåÄÖÅ ]+$/; const lengthValid = trimmed.length >= 10 && trimmed.length <= 50; const charactersValid = allowedPattern.test(trimmed); return lengthValid && charactersValid; }
...
function attachResourceDescriptionValidation(input) { const update = () => { const raw = input.value; if (raw.trim() === "") { setInputVisualState(input, "neutral"); updateCreateButtonState(); return; } const valid = isResourceDescriptionValid(raw); setInputVisualState(input, valid ? "valid" : "invalid"); updateCreateButtonState(); }; input.addEventListener("input", update); update(); }
...
function updateCreateButtonState() { const nameValid = isResourceNameValid(resourceNameInput.value); const descValid = isResourceDescriptionValid(resourceDescriptionInput.value); setButtonEnabled(createButton, nameValid && descValid); }