...
const resourceValidators = [
  ...
  body('resourceName')
    ...
    .trim()
    .isLength({ min: 5, max: 30 }).withMessage('Resource name must be 5-30 characters') //määrittele hyväksytty pituus
    .escape(),
  ...
  body('resourceDescription')
    ...
    .escape(),//estää merkit esim. <script>

  body('resourceAvailable')
    //.exists({ checkFalsy: true }).withMessage('resourceAvailable is required') tämä ei hyväksy falsea
  ...
  body('resourcePriceUnit')
    ...
    .trim()
    .isIn(['hour', 'day', 'week', 'month'])//lisätty week ja month
...
app.post('/api/resources', resourceValidators, async (req, res) => {
  ...
  try {
    ...
    `;
    const params = [
      resourceName, //nimi pysyy nimenä
      resourceDescription,
      Boolean(resourceAvailable),
      Number(resourcePrice), //ei kaksinkertaista hintaa
      resourcePriceUnit
    ];
